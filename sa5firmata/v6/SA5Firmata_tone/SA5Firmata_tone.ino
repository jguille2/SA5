/*
  SA5Firmata_tone version 6 (is the "tone" version, not supporting IR features but with the full tone function)
  Pins 2 (v2), 5 (v5 tone and ir) and 3 (v6 supporting neopixels) are marked as SHIFT mode ( pin 6 is only for v5/6 IR version)
  v2 Jun 2016 first SA5Firmata
  v5 Oct 2018 extending functions
  v6 Aug 2025 adding neopixels support
...
#define NEOPIXEL 0xD1
#define NEOPIXEL_REGISTER 0xD0
#define MAX_NEO 1
    ...
    case CAPABILITY_QUERY:
      ...
      for (byte pin = 0; pin < TOTAL_PINS; pin++) {
        if (IS_PIN_DIGITAL(pin)) {
          ...
          Firmata.write(1);
//////////////////////////////////////////////////
//SA5_tone version 6. Marking digital pins 2, 3 and 5 
    ...
    case NEOPIXEL:
      {
        if (neopixels) {
          ...
          int index = argv[1];
          if (index != 0) {
            int leds = neopixels->numPixels();
            if (sign == 1) {
              index = (index % leds) -1;
              if (index < 0) {index = leds -1;}
            } else {
              index = leds - (index % leds);
              if (index == leds) {index = 0;}
            }
            int red = argv[2];
            if (red < 0) {red = 0;}
            if (red > 255) {red = 255;}
...
//
// Ending 14 sysex commands for SA5Firmata_tone
//////////////////////////////////////////////////
