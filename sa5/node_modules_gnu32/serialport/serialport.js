  ...
  function listUnix(callback) {
    ...
    fs.readdir(dirName, function (err, files) {
      ...
      for (var i = files.length - 1;i>=0;i--){
        if ((files[i].indexOf('rfcomm') === -1 && files[i].indexOf('ttyS') === -1 && files[i].indexOf('ttyACM') === -1 && files[i].indexOf('ttyUSB') === -1 && files[i].indexOf('ttyAMA') === -1) || !fs.statSync(path.join(dirName,files[i])).isCharacterDevice()){
          files.splice(i,1);
        }
      }
